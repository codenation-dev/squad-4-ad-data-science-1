...
df = data_treatment.fetch_market(1)
...
rec_sys_output.to_csv(path+'CL_rank_01.csv', index=False)
...
print("==> GENERATING DATASETS FOR TRAINING YOUR MODEL")
# Run Treatment and load data:
df = data_treatment.fetch_market(3)
# Transform index to column
df.reset_index(level=0, inplace=True)
df.head(2)
# Generate Dataframes:
...
rec_sys_output = recommendation[['id','prob']]
path = r'../recommender_system_leads/predicts/'
rec_sys_output.to_csv(path+'CL_rank_03.csv', index=False)